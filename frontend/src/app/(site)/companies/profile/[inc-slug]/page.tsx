...
const StockProfile = ({ params }: Props) => {
  ...
  const chartBlock = (
    <div className={`overflow-hidden rounded-xl ${CMC.card}`}>
      ...
      <div className="h-[min(480px,65vh)] min-h-[360px] w-full p-1.5">
        {chartLoading ? (
          <div className={`flex h-full items-center justify-center gap-2 ${CMC.muted}`}>
          ...
          <div className={`flex h-full items-center justify-center text-sm ${CMC.muted}`}>No price data for this range.</div>
        )}
      </div>
  ...
  return (
    <div className={`min-h-screen ${CMC.bg} ${CMC.text} font-sans antialiased`}>
      <div className="mx-auto max-w-[1600px] px-3 py-4 sm:px-4 md:py-6 lg:px-6">
        <nav className={`mb-4 flex flex-wrap items-center gap-1 text-xs ${CMC.muted}`} aria-label="Breadcrumb">
        ...
        <div className="grid grid-cols-1 gap-4 xl:grid-cols-[minmax(0,280px)_minmax(0,1fr)_minmax(0,300px)] xl:items-start xl:gap-5">
            ...
            <div className={`rounded-xl ${CMC.card} p-4`}>
              <div className="flex gap-3">
                ...
                <div className="min-w-0 flex-1">
                  <h1 className="text-base font-bold leading-snug text-white">{details?.company_name || symbol}</h1>
              ...
              <div className="mt-4">
                ...
                <p className={`mt-2 flex items-center gap-1 text-[10px] ${CMC.muted}`}>
          ...
          {/* —— Center column: CMC-style tabs + main surface —— */}
            ...
            {mainTab === "markets" && (
              <section className={`rounded-xl ${CMC.card} overflow-hidden`}>
                ...
                <h2 className="px-4 pt-4 text-lg font-bold text-white">
                  {details?.company_name || symbol} <span className={CMC.muted}>markets</span>
                </h2>
                ...
                {news.length === 0 ? (
                  ...
                  news.map((item, idx) => (
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className={`block rounded-xl ${CMC.card} p-4 transition hover:border-[#3861fb]/45`}
                    >
                      <div className="mb-2 flex justify-between gap-2 text-[10px]">
                      ...
                      <p className={`mt-2 text-xs leading-relaxed ${CMC.muted}`}>{truncateText(item.content || "", 280)}</p>
                    </a>
                  ))
                )}
              </section>
              ...
              <div className="space-y-4">
                <section className={`rounded-xl ${CMC.card} p-5`}>
                  <div className="mb-3 flex items-center gap-2">
                    ...
                    <h2 className="text-base font-bold text-white">About {details?.company_name || symbol}</h2>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
                    {similar.map((comp: any, idx: number) => (
                      ...
                      >
                        ...
                        <span className={`mt-0.5 line-clamp-2 text-[10px] ${CMC.muted}`}>{comp.company_name}</span>
                      </Link>
                    ))}
                  </div>
                </section>
          ...
          {/* —— Right column: sentiment + feed (CMC community rail) —— */}
          <aside className="order-3 flex flex-col gap-4 xl:sticky xl:top-4 xl:max-h-[calc(100vh-1.5rem)] xl:overflow-y-auto">
            <div className={`rounded-xl ${CMC.card} p-4`}>
              <div className="flex items-start justify-between gap-2">
                <div>
                  <p className="text-sm font-bold text-white line-clamp-1">{details?.company_name || symbol}</p>
                    ...
                    <div className="flex gap-2">
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center justify-between gap-1">
                          <span className="text-xs font-bold text-white">DSA Feed</span>
