...
class StockService
{
    ...
    /**
     * Side API: Fetch expert advices from AlphaVantage
     */
    public function ExpertAdvices($symbol)
    {
        $symbol = strtoupper($symbol);
        $key = env("ALPHA_VANTAGE_API_KEY");
        Log::info(message: "Fetching stock attributes for symbol: " . $symbol . " using AlphaVantage key: " . ($key ? 'SET' : 'MISSING'));
        if (!$key) {
