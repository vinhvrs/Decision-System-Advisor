...
#             query = """
#                 SELECT i.symbol FROM instrument_snapshot s
...
#                 rows = cur.fetchall()
#             conn.close()
#             return [r['symbol'].upper() if isinstance(r, dict) else r[0].upper() for r in rows]
#         except Exception as e:
...
#             conn = pymysql.connect(**Config.DB_CONFIG)
#             query = """
#                 SELECT d.open, d.high, d.low, d.close, d.timestamps
...
#         if df.empty or len(df) < 35: # MACD cần tối thiểu 35 nến để ổn định
#             print(f"⚠️ {symbol}: Không đủ dữ liệu ({len(df)} nến)")
#             return

#         try:
#             # --- TÍNH TOÁN CÁC CHỈ BÁO (Khớp với PHP Service) ---
#             # 1. SMA & EMA (14)
...
#             # Khử NaN để tránh lỗi JSON
#             df.fillna(0, inplace=True)
#             last = df.iloc[-1]

#             # --- GOM NHÓM DỮ LIỆU PHẲNG (Dưới summary) ---
#             # Lưu ý: pandas_ta đặt tên cột tự động theo chuẩn: SMA_14, EMA_14, RSI_14, ...
#             result = {
#                 "symbol": symbol,
#                 "price": float(last['close']),
#                 "indicators": {
#                     "sma": round(float(last.get('SMA_14', 0)), 2),
#                     "ema": round(float(last.get('EMA_14', 0)), 2),
#                     "rsi": round(float(last.get('RSI_14', 0)), 2),
#                     "macd": {
#                         "macd": round(float(last.get('MACD_12_26_9', 0)), 6),
#                         "signal": round(float(last.get('MACDs_12_26_9', 0)), 6),
#                         "histogram": round(float(last.get('MACDh_12_26_9', 0)), 6)
#                     },
#                     "bollinger_bands": {
#                         "lower": round(float(last.get('BBL_20_2.0', 0)), 2),
#                         "middle": round(float(last.get('BBM_20_2.0', 0)), 2),
#                         "upper": round(float(last.get('BBU_20_2.0', 0)), 2)
#                     },
#                     "stochastic": {
#                         "k": round(float(last.get('STOCHk_14_3_3', 0)), 2),
#                         "d": round(float(last.get('STOCHd_14_3_3', 0)), 2)
#                     }
#                 },
#                 "updated_at": time.strftime('%Y-%m-%d %H:%M:%S')
#             }

#             # Lưu vào Redis: summary:NVDA
...
class IndicatorService:
    ...
    def process_and_cache(self, symbol: str):
        ...
        try:
            ...
            df.ta.macd(fast=12, slow=26, signal=9, append=True)
            df.ta.bbands(length=20, std=2.0, append=True)
            print([c for c in df.columns if "BB" in c.upper()])
            ...
            # --- Bollinger Bands ---
            bbl_prev = self._safe_float(prev.get("BBL_20_2.0"))
            bbm_prev = self._safe_float(prev.get("BBM_20_2.0"))
            bbu_prev = self._safe_float(prev.get("BBU_20_2.0"))

            bbl_curr = self._safe_float(last.get("BBL_20_2.0"))
            bbm_curr = self._safe_float(last.get("BBM_20_2.0"))
            bbu_curr = self._safe_float(last.get("BBU_20_2.0"))
            ...
            print(
                ...
                f"SMA20/50 trend: {result['indicators']['ma_signals']['sma_20_50']['trend']} | "
                f"EMA12/26 trend: {result['indicators']['ma_signals']['ema_12_26']['trend']}"
            )
