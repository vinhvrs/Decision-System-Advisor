...
class ChatBotService:
    ...
    async def handle_message(self, user_text: str, style: str = "standard") -> Dict[str, Any]:
        ...
        Flow:
        1. NLP parse ticker
        2. get analysis from Redis cache
        3. if cache miss -> call auto_analyze_service
        4. normalize output fields exactly as old structure
        """
        try:
            ctx = SmoothContext(direction="in", style_preset=style)
            inbound_result = self.smoother.smooth(user_text, ctx)
            ...
            for raw_symbol in tickers:
                ...
                if cached:
                    response_payload = self._normalize_response_payload(cached)
                else:
                    analyzed = await auto_analyze_service.analyze_symbol(
                        symbol=symbol,
                        clean_text=inbound_result.clean_text,
                    )
                    response_payload = self._normalize_response_payload(
                        analyzed.get("response", {})
                    )
    ...
    def _normalize_response_payload(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        """
        Keep output fields exactly like old version.
        """
        return {
            "recommendation": payload.get("recommendation", "HOLD"),
            "confidence": payload.get("confidence", 55),
            "message": payload.get("message", "No analysis message available."),
