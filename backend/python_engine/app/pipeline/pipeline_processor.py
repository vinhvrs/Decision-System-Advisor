...
def calculate_price_impact(pub_date, symbol):
    ...
    if price_df.empty: return 0.0, 0.0, None
    ...
    if abs((actual_trade_date - target_date).days) > 7: return 0.0, 0.0, None
...
def process_docs_to_chunks(target_symbol=None, test_mode=False):
    ...
    try:
        with conn.cursor(pymysql.cursors.DictCursor) as cursor:
            sql = "SELECT id as doc_id, symbol, title, published_at FROM knowledge_docs_temp WHERE symbol = %s AND published_at > %s ORDER BY published_at ASC"
            cursor.execute(sql, (target_symbol, last_time))
            docs = cursor.fetchall()
            if not docs: return []
            print(f"🚀 [PIPELINE] Đang xử lý {len(docs)} bài báo mới...")
            for doc in docs:
                pre, post, t_date = calculate_price_impact(doc['published_at'], doc['symbol'])
                if t_date:
                    event = {
                    ...
                    if not test_mode:
                        ...
                        cursor.execute("""
                            INSERT IGNORE INTO knowledge_inference_results (id, symbol, doc_id, title, published_at, pre_trend, post_trend, recommendation)
                            VALUES (%s, %s, %s, %s, %s, %s, %s, %s)
                        """, (event['id'], event['symbol'], event['doc_id'], event['title'], event['published_at'], pre, post, rec))
